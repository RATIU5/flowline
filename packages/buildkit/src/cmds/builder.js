...
const dtsGen = (buildTsConfig, outdir) => ({
  ...
  setup(build) {
    build.onEnd(async (result) => {
      ...
      console.log(
        `${chalk.dim(`[${date}]`)} Generating TypeScript declarations...`,
      );
      try {
        await $`tsgo --emitDeclarationOnly -p ${buildTsConfig} --outDir ./${outdir}`.text();
        console.log(
          chalk.dim(`[${date}] `) +
            chalk.green("√ Generated TypeScript declarations"),
        );
      } catch (error) {
        const msg =
...
async function clean(outdir, date, skip = []) {
  ...
  console.log(
    chalk.dim(`[${date}] `) +
      chalk.dim(`Cleaning ${files.length} files from ${outdir}`),
  );
...
export default async function builder(cmd, args) {
  ...
  if (entryPoints.length === 0) {
    throw new Error(
      `No entry points found for pattern(s): ${patterns.join(", ")}`,
    );
  }
        ...
        setup(build) {
          build.onEnd(async (result) => {
            ...
            if (result?.errors.length) {
              ...
              console.error(
                chalk.dim(`[${date}] `) + chalk.red(formatted.join("\n")),
              );
              return;
            }
            if (result.warnings.length) {
              const formattedWarns = await esbuild.formatMessages(
                result.warnings,
                {
                  kind: "warning",
                  color: true,
